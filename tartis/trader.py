    ...
    def handle_market_data_stream(self, binance_websocket_api_manager, pool, symbol, bot):
        while True:
            ...
            if self.oldest_stream_data_from_stream_buffer is False:
                ...
                self.msg = self.oldest_stream_data_from_stream_buffer.get('data')
                if self.msg:
                    ...
                    for i in self.data:
                        try:
                            ...
                            for j in ['entries', 'tps', 'sls']:
                                if sum(self.trades[i[0]][j]['percent']) != 100:
                                    self.trades[i[0]][j]['point'].append(i[4+self.offset])
                                    self.trades[i[0]][j]['percent'].append(i[5+self.offset])
                                    self.trades[i[0]][j]['filled'].append(i[6+self.offset])
                                self.offset += 3
